			...
			$cellclass="";
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						$result = $db->query("SELECT title,event_id FROM events WHERE date = '".$currentDate."' AND status = 1");
						...
						if(strtotime($currentDate) == strtotime(date("Y-m-d"))){
							...
							{ 
          					  $cellclass= $row['event_id'];
      						}
      					 $cellclass="x".$cellclass;
							echo '<li date="'.$currentDate.'" class="'.$cellclass.' date_cell">';
						}else{
